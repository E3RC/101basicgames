(function(){
  ...
  GameImplementations[slug]=async function(engine){
    engine.start('Even Wins II');
    engine.setInstructions('Even Wins II (Nim). There are 7 piles of objects (1-7 each). Players take turns removing any number of objects from a single pile. The player who takes the LAST object WINS. Computer uses optimal XOR strategy.');

    var piles = [];
    for (var i = 0; i < 7; i++) piles.push(RNG.int(1, 7));
    ...
    engine.clear();
    engine.println('EVEN WINS II (Nim)');
    engine.println('');
